...
RecordSet.prototype.dfWalk = function (start, fn) {
  ...
  ;(function dfs (record, path, fn) {
    record = data[record]
    seen[record.id] = true
    results[record.id] = fn(record, path)
    ;(record.friends || []).forEach(function (f) {
      if (!seen[f]) dfs(f, path.concat(record.id), fn)
    })
  })(start, [], fn)
...
RecordSet.prototype.bfWalk = function (start, fn) {
  ...
  while (record = queue.shift()) if (!seen[record[0]]) {
    var path = record[1]
    record = data[record[0]]
    seen[record.id] = true
    results[record.id] = fn(record, path)
    queue.push.apply(queue, (record.friends || []).map(function (r) {
      return [r, path.concat(record.id)]
    }))
  }
